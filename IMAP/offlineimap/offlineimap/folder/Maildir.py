...
import os.path, os, re, time, socket, md5, simplejson
...
class MaildirFolder(BaseFolder):
    ...
    def _scanfolder(self):
        ...
        for file in files:
            messagename, messageext = os.path.splitext(os.path.basename(file))
            assert(messageext == self.extension)
    ...
    def savemessageflags(self, uid, flags):
        oldfilename,extension = os.path.splitext(self.messagelist[uid]['filename'])
        assert(extension == self.extension)
        ...
        oldfilename = oldfilename + self.extension
